...
import { Effect, effect, pureEffect } from '../core/effect';
...
export const time = <A>(initial: A): Time<A> & { _set: (value: A) => void } => {
  ...
  const timeInstance: Time<A> & { _set: (value: A) => void } = {
    ...
    interval: (ms: number): Time<A> => {
      ...
      const originalSubscribe = intervalTime.subscribe;
      intervalTime.subscribe = (fn) => {
        subscriberCount++;
        ...
        if (subscriberCount === 1) {
          intervalId = setInterval(() => {
            ...
            if (typeof currentValue === 'number') {
              ...
              intervalTime._set({ ...currentValue as any, _tick: tickCount } as A);
            }
          }, ms);
        }
    ...
    if (subscriberCount === 1) {
      intervalId = setInterval(() => {
        ...
        if (typeof value === 'number') {
          ...
          // For objects, create a new reference
          intervalTime._set({ ...value as any, _tick: tickCount } as A);
        }
      }, ms);
    }
